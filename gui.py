...
for category, classes in category_structure.items():
    cat_frame = tk.Frame(scrollable_frame, bg="#f0f0f0")
    ...
    inner_frame = tk.Frame(cat_frame, bg="#f0f0f0")
    for class_id, table_name in classes:
        table_vars[class_id] = tk.BooleanVar(value=False)
        chk = tk.Checkbutton(inner_frame, text=table_name, variable=table_vars[class_id], command=lambda: apply_filters(), bg="#f0f0f0")
        chk.pack(anchor="w", padx=20)
    inner_frame.pack()
    cat_frame.pack(anchor="w", fill="x", pady=2)

tk.Button(scrollable_frame, text="Resetovať filtre", command=lambda: reset_filters()).pack(anchor="w", pady=10, padx=5)

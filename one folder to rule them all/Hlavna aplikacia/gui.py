...
def start(project_dir, json_path):
    ...
    db_columns = (
        ...
        "koeficient","nakup_materialu","cena_prace"
    )
    ...
    basket_tree = ttk.Treeview(
        basket_frame, columns=basket_columns, show="tree headings"
    )
    ...
    _drag = {"item": None}
    def _start(evt):   _drag["item"] = basket_tree.identify_row(evt.y)
    def _motion(evt):
        iid = _drag["item"]
        if iid:
    ...
    def _release(evt): _drag["item"] = None

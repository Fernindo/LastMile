...
def _load_logged_in_user_id(conn, db_type: str) -> int | None:
    ...
    try:
        s = simpledialog.askstring("User", "Zadaj svoje user_id (číslo):")
        if s:
            return int(s)
    except Exception:
        pass
...
def show_presets_browser(parent=None):
    ...
    tb.Button(btns, text="Obnoviť", bootstyle="secondary", command=load_presets).pack(side="left")
    tb.Button(right, text="Zavrieť", bootstyle="secondary", command=win.destroy).pack(side="bottom", pady=6)
...
def show_presets_window():
    """
    Preset Builder: behaves like the normal GUI’s left DB browser + right basket,
    ...
    tk.Label(top, text="Vyhľadávanie:").pack(side="left", padx=(4, 6))
    name_entry = tk.Entry(top, width=32)
    name_entry.pack(side="left")
    name_entry.bind("<KeyRelease>", lambda e: root.after(200, refresh_db))
    tb.Button(top, text="Obnoviť", bootstyle="secondary", command=refresh_db).pack(side="left", padx=6)
    ...
    def add_selected_to_basket():
        ...
        if not sel:
            messagebox.showwarning("Výber", "Najprv vyber položky v databáze.")
            return
    ...
    tb.Button(left, text="Pridať do košíka", bootstyle="success", command=add_selected_to_basket).pack(fill="x", pady=(6, 10), padx=2)
    ...
    def save_as_preset():
        ...
        name = simpledialog.askstring("Preset", "Názov presetu:")
    ...
    btn_bar.pack(fill="x")
    tb.Button(btn_bar, text="Uložiť ako preset", bootstyle="primary", command=save_as_preset).pack(side="left", padx=4, pady=4)
    tb.Button(btn_bar, text="Zavrieť", bootstyle="secondary", command=win.destroy).pack(side="right", padx=4, pady=4)

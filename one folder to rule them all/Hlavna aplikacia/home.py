...
def main() -> None:
    ...
    def browse_root() -> None:
        path = filedialog.askdirectory(initialdir=projects_root.get() or os.path.expanduser("~"))
        if path:
    ...
    tb.Button(top, text="Create Project", bootstyle="success", command=create_project).pack(side="right")
    ...
    tb.Button(root, text="Open", bootstyle="primary", command=open_selected).pack(pady=5)
